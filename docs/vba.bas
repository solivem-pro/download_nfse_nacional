...
Sub ImportarTodosXMLs()
    ...
    ' Ajustes por eventos (cancelamento, substituição etc.)
    ZerarValoresPorEvento
...
' Zerar Eventos
' ===============================
Private Sub ZerarValoresPorEvento()
    On Error GoTo Fim

    Dim dict As Object
    Set dict = CreateObject("Scripting.Dictionary")

    Dim wsEvt As Worksheet, wsP As Worksheet, wsT As Worksheet
    Dim tblEvt As ListObject, tblP As ListObject, tblT As ListObject
    Dim lr As ListRow
    Dim chave As String

    Set wsEvt = ThisWorkbook.Sheets("EVENTOS")
    Set wsP = ThisWorkbook.Sheets("PRESTADOS")
    Set wsT = ThisWorkbook.Sheets("TOMADOS")

    Set tblEvt = wsEvt.ListObjects("NFSE_EVENTO")
    Set tblP = wsP.ListObjects("NFSE_PRESTADOS")
    Set tblT = wsT.ListObjects("NFSE_TOMADOS")

    ' Coleta chaves da EVENTO (coluna G)
    For Each lr In tblEvt.ListRows
        chave = Trim(lr.Range(1, 7).Value)
        If chave <> "" Then
            If Not dict.exists(chave) Then dict.Add chave, True
        End If
    Next lr

    ' Zera L:N em PRESTADOS quando chave existir
    For Each lr In tblP.ListRows
        chave = Trim(lr.Range(1, 15).Value) ' coluna O
        If dict.exists(chave) Then
            lr.Range(1, 12).Resize(1, 3).Value = 0 ' L:M:N
        End If
    Next lr

    ' Zera L:N em TOMADOS quando chave existir
    For Each lr In tblT.ListRows
        chave = Trim(lr.Range(1, 15).Value) ' coluna O
        If dict.exists(chave) Then
            lr.Range(1, 12).Resize(1, 3).Value = 0 ' L:M:N
        End If
...
Private Sub AplicarHyperlink(ByVal celula As Range, ByVal caminhoPDF As String, ByVal texto As String)
    ...
    ' === IGNORA LINHA DE TOTAIS DA TABELA ===
    Dim tbl As ListObject
    Set tbl = celula.ListObject

    If Not tbl Is Nothing Then
        If tbl.ShowTotals Then
            If Not tbl.TotalsRowRange Is Nothing Then
                If celula.Row = tbl.TotalsRowRange.Row Then Exit Sub
            End If
        End If
    End If
    ' ======================================
    ...
    Dim urlFallback As String
    urlFallback = "https://adn.nfse.gov.br/danfse/" & texto
    ...
    If caminhoPDF <> "" Then
        ...
        celula.Worksheet.Hyperlinks.Add _
            ...
            ScreenTip:="Clique para abrir o PDF"
    ElseIf texto <> "" Then
        ' PDF não encontrado ? DANFSe online
        celula.Worksheet.Hyperlinks.Add _
            Anchor:=celula, _
            Address:=urlFallback, _
            TextToDisplay:=texto, _
            ScreenTip:="Abrir DANFSe no portal nacional"
    End If

    If celula.Hyperlinks.count > 0 Then
        With celula.Font
            .Color = RGB(0, 50, 180)
    ...
    For Each sheetName In arrSheets
        With ThisWorkbook.Sheets(sheetName)
            With .Cells
                .Font.Name = "Consolas"
                .Font.Size = 9
                .Interior.Pattern = xlNone   ' Sem preenchimento
            End With

            With .ListObjects(1).HeaderRowRange
                .Font.Size = 10
                .Interior.Pattern = xlNone   ' Cabeçalho também sem preenchimento
            End With
        End With
    Next sheetName
End Sub
